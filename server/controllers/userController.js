...
*/
  static async signIn(req, res) {
    ...
    try {
      ...
      const loginQuery = { text: 'SELECT * FROM users WHERE username = $1 OR email = $1', values: [username.trim()] };
      const user = await client.query(loginQuery);
      const { rows, rowCount } = user;
      if (rowCount === 0) { return res.status(401).send({ status: 401, error: 'Invalid username or password' }); }
      if (passwordHash.verify(password.trim(), rows[0].password)) {
        const token = encode(rows[0].id, rows[0].isadmin);
        const updateQuery = { text: 'UPDATE users SET lastlogin = CURRENT_DATE WHERE id = $1 RETURNING email, username, registered, lastlogin, isadmin', values: [rows[0].id] };
  ...
  static async profile(req, res) {
    ...
    try {
      ...
      const updateUserQuery = { text: 'UPDATE users SET firstname = $1, lastname = $2, phonenumber = $3, updateOn = CURRENT_DATE WHERE id = $4 RETURNING firstname, lastname, phonenumber, updateon', values: [regex(firstName), regex(lastName), regex(phoneNumber), userId] };
      const updatedUser = await client.query(updateUserQuery);
      const { rows, rowCount } = updatedUser;
      if (rowCount === 1) { return res.status(200).send({ status: 200, data: rows, message: 'User profile was updated' }); }
  ...
  static async resetPassword(req, res) {
    ...
    try {
      ...
      if (email) {
        ...
        const updateUserQuery = { text: 'UPDATE users SET password = $1, updatedon = CURRENT_DATE WHERE email = $2', values: [hashedPassword, email] };
